...
export const EventDetails: React.FC = () => {
  ...
  const loadEventFiles = async () => {
    ...
    try {
      ...
      const response = await apiClient.get<DisplayFile[]>(
        `/api/v1/files/?event=${id}`
      )
      setFiles(response)
    } catch (err: any) {
    ...
    try {
      ...
      if (err.response?.status === 400) {
        ...
        if (
          errorData.user &&
          errorData.user[0] ===
            'Этот пользователь уже участвует в данном событии'
        ) {
        ...
        setRegistrationError(
          err.message || 'Не удалось зарегистрироваться на событие'
        )
      }
    } finally {
  ...
  const activeOfflineSessions = event.offline_sessions.filter(
    (s) => s.is_active
  )

  // Сортируем сессии по времени начала
  const sortedOnlineSessions = [...activeOnlineSessions].sort(
    (a, b) =>
      new Date(a.start_time).getTime() - new Date(b.start_time).getTime()
  )

  const sortedOfflineSessions = [...activeOfflineSessions].sort(
    (a, b) =>
      new Date(a.start_time).getTime() - new Date(b.start_time).getTime()
  )
              ...
              {registrationSuccess ? (
                ...
                <>
                  ...
                  <div className="text-sm text-gray-500 mb-4">
                    <p>
                      После регистрации организатор {event.owner.full_name}{' '}
                      получит уведомление о вашей заявке.
                    </p>
                    {event.is_private && (
                      <p className="mt-2 text-yellow-600">
                        Это приватное событие. Ваша заявка будет рассмотрена
                        организатором.
                      </p>
                    )}
                  </div>
                ...
                <>
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      value={fileForm.name}
                      onChange={(e) =>
                        handleFileFormChange('name', e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  ...
                  <div className="mb-4">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleFileFormChange('category', e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    >
                  ...
                  <div className="mb-4">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        handleFileFormChange('description', e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  ...
                  <div className="mb-6">
                    <div className="flex items-center">
                      <input
                        ...
                        onChange={(e) =>
                          handleFileFormChange('is_public', e.target.checked)
                        }
                  ...
                  <div className="text-sm text-gray-500 mb-4">
                    ...
                    <p>
                      Допустимые форматы: JPG, PNG, PDF, DOC, DOCX, PPT, PPTX,
                      XLS, XLSX, ZIP, RAR, MP4, MP3, TXT
                    </p>
                  </div>
                </>
              )}
              ...
              {!uploadSuccess && (
                <>
                  ...
                  <button
                    ...
                    disabled={
                      uploadLoading || !fileForm.file || !fileForm.name.trim()
                    }
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {uploadLoading ? (
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              Зарегистрировались{' '}
                              {formatRelativeDate(
                                event.current_user_participation.registered_at
                              )}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                                ...
                                {session.link && !isParticipant && !isOwner && (
                                  <div className="md:col-span-2">
                                    <div className="flex items-center text-gray-500">
                                      ...
                                      <span>
                                        Ссылка доступна после регистрации
                                      </span>
                                    </div>
                                  </div>
                                )}
                          ...
                          {sortedOnlineSessions.length > 3 && (
                            <div className="text-center pt-4">
                              ...
                              >
                                Показать все {sortedOnlineSessions.length}{' '}
                                онлайн-сессий
                              </button>
                            </div>
                          )}
                          ...
                          {sortedOfflineSessions.length > 3 && (
                            <div className="text-center pt-4">
                              <button
                                ...
                                className="text-blue-600 hover:text-blue-800 text-sm font-medium"
                              >
                                Показать все {sortedOfflineSessions.length}{' '}
                                офлайн-сессий
                              </button>
                            </div>
                          )}
                      ...
                      {sortedOnlineSessions.map((session) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <span
                              className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ml-3 ${getStatusColor(
                                session.status
                              )}`}
                            >
                              {getStatusText(session.status)}
